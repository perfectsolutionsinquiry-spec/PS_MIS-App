...
|---|---|---|---|---|
| Shilpkaar (Aarambh) | pre-dates this runbook | — | — | Original data load, onboarded before this document existed — see `CLAUDE.md`'s "Real data loaded" for how it was verified at the time (staff login, not a real builder login — see the next row). |
| Shilpkaar (Aarambh) — builder login | 4 September 2026 | Claude, via the Clerk-production-switch work | **not done** | `builder_users` was discovered completely empty while re-linking production users — this is that builder's *first real login*, created as a side effect, not a deliberate onboarding run. Steps 1–3 done (Clerk production user created, `builder_users` row inserted and linked). **Step 4 was not performed** — nobody has actually signed in as this login yet. It's also not fully testable as written yet: the "search for a known other-builder customer name, must return zero" check needs a *second* builder's data to exist, and Shilpkaar is currently the only builder in the system. Treat this row as open until someone actually signs in and confirms it renders sane, Shilpkaar-only data. |
