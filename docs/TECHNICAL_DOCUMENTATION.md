...
| `POST /customers` | Creates a customer from `NewCustomerModal.tsx`'s form fields. Staff must pass `builder_id`; a builder identity always uses their own. | required | Yes |
...
| `CustomersScreen.tsx` | Supplies `CUSTOMER_COLUMNS` (name/phone/email/stage, with the stage-badge renderer) to `DataTable`. Owns the "New customer" modal's open/close state. | — (all fetching happens in `App.tsx`) |
| `DataTable.tsx` | **The generic list engine.** Sortable columns, per-column Contains/Starts-with filter, gear-icon column-visibility slushbucket (saved to `localStorage`), optional "New" button. Every future table screen should plug into this rather than building its own table. | — |
| `NewCustomerModal.tsx` | The "New" button's form — name/agreement no./phone/email/stage, plus a builder picker shown only to staff. | `/builders` (staff only), `POST /customers` |
| `CustomerDetailScreen.tsx` | The full customer record: summary tiles, 5 field sections (identity/dates/pricing/funding/status, each togglable edit↔view), read-only co-applicants, record-a-payment form, payment history table, milestone table. Also exports `STAGE_OPTIONS` (shared with `NewCustomerModal.tsx`). | `GET /customers/:id`, `GET /banks`, `PATCH /customers/:id`, `POST /customers/:id/payments` |
