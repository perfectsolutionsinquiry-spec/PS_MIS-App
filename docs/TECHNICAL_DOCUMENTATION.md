...
| `auth.ts` | Consumes the provider-neutral identity verification boundary, resolves local identity, attaches role capabilities, and provides the server-side capability pre-handler. | `identity-provider.ts`, staff/builder identity lookup |
