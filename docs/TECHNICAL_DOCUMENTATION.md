...
| `App.tsx` | Top-level shell: signed-out vs signed-in, fetches `/me` + the customer list + `/dashboard/overview` on load, owns URL-backed customer list/detail navigation, and renders `Sidebar` + the active screen. Customer detail values are fetched by `CustomerDetailScreen` only after a record is opened. `loadCustomers()` returns the fresh array (not just setting state) so a caller mid-function — e.g. after creating a customer — can read the new row immediately rather than the stale pre-refresh `customers` closure. | `/me`, `/customers`, `/dashboard/overview` |
