...
| `index.css` | The one global CSS file in an otherwise fully-inline-styled app — loads IBM Plex Sans, sets it as the `body` default, and provides responsive breakpoints for the advanced filter dialog so condition fields/options reflow on narrow screens. | — |
...
| `PageHeader.tsx` | Shared all-caps title and actions. Customers also supplies the total row count and a refresh button that re-fetches only `/customers`; email/export remain disabled ("soon"). | — |
...
| `DataTable.tsx` | **The generic list engine.** Sortable columns (in `visibleKeys`' own order — the slushbucket's ↑/↓ reordering changes this, not just visibility), per-column Contains/Starts-with filter (portaled to `document.body`, viewport-edge-aware), advanced funnel filter builder with inline `and`/`or` controls and a row-end trash/delete control, condition sets, richer operators, grouping, multi-level sort rows with direction toggles and a `None` state, gear-icon column-visibility slushbucket (saved to `localStorage`), optional "New" button. Active conditions in one set are evaluated as AND against the same row; sets are OR'ed. Blank value rows are ignored. Filtering/grouping/sorting operate client-side on rows already authorized and fetched by the API; every future table screen should plug into this rather than building its own table. | — |
